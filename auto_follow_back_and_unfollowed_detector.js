// ==UserScript==
// @name        nameMC auto follow back & unfollow detector
// @namespace   Violentmonkey Scripts
// @match       https://namemc.com/profile/*
// @grant       none
// @version     1.0.0
// @author      Zapto
// @description 4/16/2022, 5:25:16 PM
// ==/UserScript==

if(document.location.pathname.endsWith("followers")) {
...
//var table_childs = document.querySelector("body > main > div:nth-child(6) > div > div > table").children[0]
//table_childs.removeChild(table_childs.children[0])

$('body > main > div.mb-3').append('<div class="text-center mb-3"><button type="button" class="btn btn-success" onclick="saveToCache()">Save to cache</button></div>')

window.saveToCache = function () {
  window.localStorage.followers_state = "1"
  window.localStorage.followers = '[]'
  $('body > main > div:nth-child(4) > div > nav:nth-child(1) > ul > li:nth-child(1) > a')[0].click()
}

window.getCache = function () {
  if (window.localStorage.followers) {
    return JSON.parse(window.localStorage.followers)
  } else return []
}

window.saveCache = function (array) {
  window.localStorage.followers = JSON.stringify(array)
}

window.extractedArray = $('table').tableToJSON({
...
if(!window.localStorage.followers_state) {
  window.localStorage.followers_state = "1"
  window.localStorage.followers = '[]'
} else if (window.localStorage.followers_state == "1") {
  var array = window.getCache()
  window.extractedArray.forEach(function(el) {array.push(el.Profile)})
  window.saveCache(array)
  if(!$("body > main > div:nth-child(4) > div > nav:nth-child(1) > ul > li:nth-child(4)").hasClass("disabled")) {
    $("body > main > div:nth-child(4) > div > nav:nth-child(1) > ul > li:nth-child(4)").children(":first")[0].click()
  } else {
    window.localStorage.followers_state = "0"
    $('body > main > div:nth-child(4) > div > nav:nth-child(1) > ul > li:nth-child(1) > a')[0].click()
  }
}
...
if(document.location.pathname.endsWith("following")) {
  if(window.localStorage.followers) {
    document.querySelectorAll("body > main > div:nth-child(6) > div > div > table > tbody > tr").forEach(function(user) {
    if(window.localStorage.followers.includes(user.querySelector(" td:nth-child(3) > a").title)) {
        
    } else {
        var button = user.querySelector("td:nth-child(7) > button")
